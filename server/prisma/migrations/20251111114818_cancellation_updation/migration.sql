/*
  ...
  - You are about to alter the column `paymentStatus` on the `booking` table. The data in that column could be lost. The data in that column will be cast from `VarChar(191)` to `Enum(EnumId(10))`.
...
-- AlterTable
ALTER TABLE `booking` ADD COLUMN `refundStatusUpdatedAt` DATETIME(3) NULL,
    MODIFY `paymentStatus` ENUM('PENDING', 'PAID', 'FAILED', 'REFUND_INITIATED', 'REFUND_COMPLETED', 'REFUND_FAILED', 'REFUND_NOT_APPLICABLE') NOT NULL DEFAULT 'PENDING';
