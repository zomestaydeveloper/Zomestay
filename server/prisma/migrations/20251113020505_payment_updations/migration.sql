/*
  ...
  - You are about to alter the column `status` on the `payment` table. The data in that column could be lost. The data in that column will be cast from `VarChar(191)` to `Enum(EnumId(14))`.
...
-- AlterTable
ALTER TABLE `payment` MODIFY `status` ENUM('PENDING', 'PAID', 'FAILED', 'REFUND_INITIATED', 'REFUND_COMPLETED', 'REFUND_FAILED', 'REFUND_NOT_APPLICABLE') NOT NULL DEFAULT 'PENDING';
