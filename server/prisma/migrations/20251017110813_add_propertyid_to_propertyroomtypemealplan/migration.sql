/*
  ...
  - Added the required column `propertyId` to the `PropertyRoomTypeMealPlan` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `propertyroomtypemealplan` ADD COLUMN `propertyId` CHAR(36) NOT NULL;

-- CreateIndex
CREATE INDEX `PropertyRoomTypeMealPlan_propertyId_idx` ON `PropertyRoomTypeMealPlan`(`propertyId`);
