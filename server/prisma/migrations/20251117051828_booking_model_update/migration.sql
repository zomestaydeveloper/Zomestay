/*
  ...
  - You are about to drop the column `roomSelections` on the `booking` table. All the data in the column will be lost.
...
-- AlterTable
ALTER TABLE `booking` DROP COLUMN `roomSelections`;

-- CreateTable
CREATE TABLE `booking_room_selections` (
    `id` CHAR(36) NOT NULL,
    ...
    `datesReserved` JSON NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,

    INDEX `booking_room_selections_bookingId_idx`(`bookingId`),
    INDEX `booking_room_selections_roomTypeId_idx`(`roomTypeId`),
    INDEX `booking_room_selections_mealPlanId_idx`(`mealPlanId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateIndex
CREATE INDEX `order_room_selections_mealPlanId_idx` ON `order_room_selections`(`mealPlanId`);
...
-- AddForeignKey
ALTER TABLE `Booking` ADD CONSTRAINT `Booking_agentId_fkey` FOREIGN KEY (`agentId`) REFERENCES `TravelAgent`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `order_room_selections` ADD CONSTRAINT `order_room_selections_orderId_fkey` FOREIGN KEY (`orderId`) REFERENCES `Order`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `booking_room_selections` ADD CONSTRAINT `booking_room_selections_bookingId_fkey` FOREIGN KEY (`bookingId`) REFERENCES `Booking`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `booking_room_selections` ADD CONSTRAINT `booking_room_selections_roomTypeId_fkey` FOREIGN KEY (`roomTypeId`) REFERENCES `PropertyRoomType`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `booking_room_selections` ADD CONSTRAINT `booking_room_selections_mealPlanId_fkey` FOREIGN KEY (`mealPlanId`) REFERENCES `MealPlan`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Payment` ADD CONSTRAINT `Payment_agentId_fkey` FOREIGN KEY (`agentId`) REFERENCES `TravelAgent`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `PropertyRoomTypeAmenity` ADD CONSTRAINT `PropertyRoomTypeAmenity_propertyRoomTypeId_fkey` FOREIGN KEY (`propertyRoomTypeId`) REFERENCES `PropertyRoomType`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `PropertyRoomTypeAmenity` ADD CONSTRAINT `PropertyRoomTypeAmenity_amenityId_fkey` FOREIGN KEY (`amenityId`) REFERENCES `Amenity`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
