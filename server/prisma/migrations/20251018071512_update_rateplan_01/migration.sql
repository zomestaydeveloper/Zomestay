/*
  ...
  - You are about to drop the column `ratePlanColor` on the `propertyroomtypemealplan` table. All the data in the column will be lost.
  - You are about to drop the column `ratePlanName` on the `propertyroomtypemealplan` table. All the data in the column will be lost.
  - A unique constraint covering the columns `[ratePlanId,propertyRoomTypeId,mealPlanId]` on the table `PropertyRoomTypeMealPlan` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `ratePlanId` to the `PropertyRoomTypeMealPlan` table without a default value. This is not possible if the table is not empty.
...
-- DropIndex
DROP INDEX `PropertyRoomTypeMealPlan_propertyRoomTypeId_mealPlanId_rateP_key` ON `propertyroomtypemealplan`;

-- DropIndex
DROP INDEX `PropertyRoomTypeMealPlan_ratePlanName_idx` ON `propertyroomtypemealplan`;

-- DropIndex
DROP INDEX `special_rates_hostId_fkey` ON `special_rates`;

-- AlterTable
ALTER TABLE `propertyroomtypemealplan` DROP COLUMN `ratePlanColor`,
    DROP COLUMN `ratePlanName`,
    ADD COLUMN `ratePlanId` CHAR(36) NOT NULL;
...
CREATE TABLE `RatePlan` (
    ...
    INDEX `RatePlan_propertyId_idx`(`propertyId`),
    INDEX `RatePlan_isDeleted_idx`(`isDeleted`),
    UNIQUE INDEX `RatePlan_propertyId_name_isDeleted_key`(`propertyId`, `name`, `isDeleted`),
    UNIQUE INDEX `RatePlan_propertyId_color_isDeleted_key`(`propertyId`, `color`, `isDeleted`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateIndex
CREATE INDEX `PropertyRoomTypeMealPlan_ratePlanId_idx` ON `PropertyRoomTypeMealPlan`(`ratePlanId`);

-- CreateIndex
CREATE UNIQUE INDEX `PropertyRoomTypeMealPlan_ratePlanId_propertyRoomTypeId_mealP_key` ON `PropertyRoomTypeMealPlan`(`ratePlanId`, `propertyRoomTypeId`, `mealPlanId`);
