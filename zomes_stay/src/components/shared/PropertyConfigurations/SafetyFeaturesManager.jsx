...
const buildIconUrl = (iconPath) => {
  ...
  const relative = iconPath.startsWith("/") ? iconPath : `/${iconPath}`;
...
const SafetyFeaturesManager = ({
  title = "Safety & Hygiene",
  description = "Manage safety and hygiene features highlighted on property listings.",
  allowCreate = true,
  allowEdit = true,
  allowDelete = true,
  ...
  const refreshList = async () => {
    setIsLoading(true);
    try {
      const response = await propertyService.getSafeties();
      ...
      if (payload?.success) {
        ...
        setFeedback({
          isOpen: true,
          type: "error",
          title: "Failed to load safety features",
          message: payload?.message || "Unable to fetch safety & hygiene items.",
        });
      }
    } catch (error) {
      setFeedback({
  ...
  const openEditModal = (item) => {
    ...
    setFormState({
      ...
      iconFile: null,
      iconPreview: buildIconUrl(item.icon),
    });
    setFormErrors({});
    setIsModalOpen(true);
  };
  ...
  const handleToggleStatus = async (item) => {
    ...
    try {
      await propertyService.updateSafety(item.id, formData);
      setFeedback({
        isOpen: true,
        type: "success",
        title: "Safety feature updated",
        message: `${item.name} is now ${!item.isActive ? "active" : "inactive"}.`,
      });
      await refreshList();
    } catch (error) {
      setFeedback({
        isOpen: true,
        type: "error",
        title: "Failed to update safety feature",
        message: getErrorMessage(error),
      });
    }
  };
  ...
  const handleDelete = async (item) => {
    ...
    const confirmed = window.confirm(`Delete safety feature "${item.name}"?`);
    if (!confirmed) return;

    try {
      await propertyService.deleteSafety(item.id);
      setFeedback({
        isOpen: true,
        type: "success",
        title: "Safety feature deleted",
      ...
      setFeedback({
        isOpen: true,
        type: "error",
        title: "Failed to delete safety feature",
        message: getErrorMessage(error),
      });
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (formState.id) {
        await propertyService.updateSafety(formState.id, formData);
        setFeedback({
          isOpen: true,
          type: "success",
          title: "Safety feature updated",
          message: "Safety & hygiene details were saved successfully.",
        });
      } else {
        await propertyService.createSafety(formData);
        setFeedback({
          isOpen: true,
          type: "success",
          title: "Safety feature created",
          message: "Safety & hygiene item was added successfully.",
        });
      }
      ...
      setFeedback({
        isOpen: true,
        type: "error",
        title: formState.id ? "Failed to update safety feature" : "Failed to create safety feature",
        message: getErrorMessage(error),
      });
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
        {isLoading ? (
          <CardLoader text="Loading safety features..." />
        ) : items.length === 0 ? (
          <div className="p-8 text-center text-sm text-gray-500">
            No safety or hygiene items found.{" "}
            {allowCreate ? 'Click "Add Safety Feature" to create your first entry.' : "Please check again later."}
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-4 py-3 text-gray-700">{categoryLabel(item.category)}</td>
                    <td className="px-4 py-3">
                      {item.icon ? (
                        <img
          ...
          <div className="w-full max-w-lg rounded-lg bg-white shadow-xl">
            <form onSubmit={handleSubmit} className="space-y-6 p-6">
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">
                    {formState.id ? "Edit Safety Feature" : "Add Safety Feature"}
                  </h2>
                  <p className="text-xs text-gray-500">
                    Provide the safety feature name and SVG icon.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="flex justify-end gap-2">
                ...
                >
                  {isSubmitting ? "Saving..." : formState.id ? "Save changes" : "Create safety feature"}
                </button>
              </div>
            </form>
          </div>
...
export default SafetyFeaturesManager;
