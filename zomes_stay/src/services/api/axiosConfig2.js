...
axiosInstance.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 401) {
      // Special case: The property check during login might fail (e.g. no property yet)
      // We do NOT want to log the user out in this case.
      if (error.config?.url && error.config.url.includes('/propertiesbyhost')) {
        console.warn("Ignored 401 from property check to avoid logout loop");
        return Promise.reject(error);
      }
